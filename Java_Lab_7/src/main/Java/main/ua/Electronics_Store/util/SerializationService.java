...
public class SerializationService {
    ...
    // --- Object (single POJO) support ---
    public <T> void saveObjectToJson(T object, File file) {
        try {
            file.getParentFile().mkdirs();
            jsonMapper.writerWithDefaultPrettyPrinter().writeValue(file, object);
            logger.info("Saved JSON object to: " + file.getAbsolutePath());
        } catch (IOException e) {
            logger.severe("Failed saving JSON object to: " + file.getAbsolutePath() + " — " + e.getMessage());
            throw new DataSerializationException("Failed to save JSON object", e);
        }
    }

    public <T> T loadObjectFromJson(File file, Class<T> type) {
        try {
            T obj = jsonMapper.readValue(file, type);
            logger.info("Loaded JSON object from: " + file.getAbsolutePath());
            return obj;
        } catch (IOException e) {
            logger.severe("Failed loading JSON object from: " + file.getAbsolutePath() + " — " + e.getMessage());
            throw new DataSerializationException("Failed to load JSON object", e);
        }
    }

    public <T> void saveObjectToYaml(T object, File file) {
        try {
            file.getParentFile().mkdirs();
            yamlMapper.writerWithDefaultPrettyPrinter().writeValue(file, object);
            logger.info("Saved YAML object to: " + file.getAbsolutePath());
        } catch (IOException e) {
            logger.severe("Failed saving YAML object to: " + file.getAbsolutePath() + " — " + e.getMessage());
            throw new DataSerializationException("Failed to save YAML object", e);
        }
    }

    public <T> T loadObjectFromYaml(File file, Class<T> type) {
        try {
            T obj = yamlMapper.readValue(file, type);
            logger.info("Loaded YAML object from: " + file.getAbsolutePath());
            return obj;
        } catch (IOException e) {
            logger.severe("Failed loading YAML object from: " + file.getAbsolutePath() + " — " + e.getMessage());
            throw new DataSerializationException("Failed to load YAML object", e);
        }
    }
}
