...
public class Main {
    public static void main(String[] args) {
        ...
        Product product_1 = new Product("Product_1",new Category("Phone","nice phone"),1500,43);

        product_1.setStatus(Product.ProductStatus.IN_STOCK);

        String produktst;

        switch (product_1.getStatus()) {
            case IN_STOCK:
                System.out.println("Продукт є в наявності");
                break;
            case OUT_OF_STOCK:
                System.out.println("Продукту немає в наявності");
                break;
            case DISCONTINUED:
                System.out.println("Продукт знято з виробництва");
                break;
        }


        List<Product> orderProducts = new ArrayList<>();
        orderProducts.add(product_1);

        Order order_1 = new Order(
                new Customer("Bob", "Smit", "dabidabi@du"),
                orderProducts,
                LocalDate.of(2025, 12, 1)
        );


        order_1.setStatus(Order.OrderStatus.PENDING);

        String message = switch (order_1.getStatus()) {
            case PENDING -> "Замовлення очікує обробки.";
            case PROCESSING -> "Замовлення зараз обробляється.";
            case SHIPPED -> "Замовлення відправлено клієнту.";
            case DELIVERED -> "Замовлення успішно доставлено!";
            case CANCELED -> "Замовлення було скасовано.";
        };
